...
function 更新视图(event, 路径) {
    // 处理在 <a> 或 <img> 上触发事件的差异
    const 目标标识符 = event.target.firstChild || event.target;

    const 显示新图片 = () => {
        图库图片.src = 路径;
        图库标题.textContent = 目标标识符.alt;
    };
    ...
    // 开始一次视图过渡：
    document.startViewTransition(() => 显示新图片());
}
...
function 组件生成(数据源) {
        ...
        图片标签 = `
            <span class="img mdui-ripple mdui-ripple-white" onload="cssd()" style="background:${动态颜色值}"mdui-tooltip="{content: '${图片简介}'}">
                <a href="/PhotoInfo/${图片地址}">
                    <img id="imgname_${图片地址}" src="../img/主站压缩图片//${图片地址}" title=${图片简介} style="opacity: 0" target="_blank">
                </a>
                <p>${图片简介_剪切}</p>
            </span>
        `
    ...
    // 加入View Transition API
    for (a in 数据源) {
        图片地址 = 数据源[a][0]
        document.getElementById(`imgname_${图片地址}`).addEventListener('click', function (event) {
            // 在事件处理函数中可以直接访问事件对象 event
            更新视图(event, `../img/主站压缩图片//${图片地址}`)
