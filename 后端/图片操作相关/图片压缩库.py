...
def ResizeImage(输入路径, 输出文件, 目标大小=30, 压缩质量=95, 压缩率=0.9, 原图覆盖基准=4096):  # 通常你只需要修改目标大小
    ...
    try:
        ...
        os.remove(输入路径)
        print(f'图片压缩器：打开图片 {输入路径} 失败，删除这种图片 :{e}')
        return 0
    ...
    while 原始大小 > 目标大小:
        图片 = Image.open(输出文件)
        宽度, 高度 = 图片.size
        压缩后图片 = 图片.resize((int(宽度 * 压缩率), int(高度 * 压缩率)))  # 最后一个参数设置可以提高图片转换后的质量
        计数器 = 计数器 + 1
        print(
            f'图片压缩库：压缩图片 {输入路径} 大小 {压缩后图片.size} 超过目标大小 {目标大小}，继续压缩，当前压缩次数 {计数器}')

        if 计数器 > 100:
            # 避免死循环
            os.remove(输入路径)
            print(f'图片压缩器：图片压缩器故障（死循环） {输入路径}  ，删除这张图片')
            break
        try:
