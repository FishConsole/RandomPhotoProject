...
def 网易云爬虫_本地音乐提供器():
    ...
    文件组 = os.listdir(os.path.join(路径控制.前端资源目录.静态资源目录(), 'bgm'))
    结果 = []
    if len(文件组) == 0:
        try:
            网易云爬虫_音乐下载器()
            ...
            print(
                f'网易云爬虫：服务器环境异常，可能是因为Chrome和Chromedriver不存在，请将其安装以激活网易云音乐爬虫服务，如无视该错误，将会放慢index页面加载速度。你也可以上传一个MP3'
